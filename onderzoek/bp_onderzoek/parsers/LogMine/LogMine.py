...
class LogParser():
    ...
    def parse(self, logname):
        ...
        for lev in range(self.levels):
            if lev == 0:
                # Clustering
                self.level_clusters[0] = self.get_clusters(
                    self.df_log['Content_'], lev)
            else:
                # Clustering
                ...
                for cluster in clusters:
                    cluster.patterns = [
                        self.sequential_merge(cluster.patterns)]
    ...
    def get_clusters(self, logs, lev, old_clusters=None):
        ...
        for logidx, log in enumerate(logs):
            match = False
            for cluster in clusters:
                dis = self.msgDist(log, cluster.patterns[0]) if lev == 0 else self.patternDist(
                ...
                if dis and dis < self.max_dist:
                    if lev == 0:
                        ...
                        cluster.patterns.append(
                            old_clusters[logidx].patterns[0])
                    match = True
    ...
    def load_data(self):
        ...
        self.df_log = self.log_to_dataframe(os.path.join(
            self.path, self.logname), regex, headers, self.logformat)
